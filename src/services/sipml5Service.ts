...
export class SipML5Service {
  ...
  private updateCallState(sessionId?: string, status?: CallState['status']): void {
    ...
    if (finalStatus === 'connecting') {
      ...
      if (!callInfo.connectedTime) {
        ...
        console.log('SipML5: Call connected at:', callInfo.connectedTime, 'for session:', targetSessionId);
      }
    ...
    if (this.isConferenceMode) {
      ...
      for (const [sessionId, audio] of this.sessionAudioElements.entries()) {
        ...
        if (callInfo) {
          ...
          console.log(`SipML5 Conference mode: Audio ${shouldMute ? 'muted' : 'unmuted'} for session ${sessionId} (onHold: ${callInfo.isOnHold})`);
        }
      }
    } else {
      ...
      for (const [sessionId, audio] of this.sessionAudioElements.entries()) {
        ...
        if (callInfo) {
          ...
          audio.muted = shouldMute;
          console.log(`SipML5 Normal mode: Audio ${shouldMute ? 'muted' : 'unmuted'} for session ${sessionId} (active: ${sessionId === this.activeSessionId}, onHold: ${callInfo.isOnHold})`);
        }
      }
    }
  }
  ...
  private setupAudioContext() {
    ...
    if (this.audioContext.state === 'suspended') {
      this.audioContext.resume().then(() => {
        console.log('SipML5: AudioContext resumed successfully');
      }).catch(error => {
        console.warn('SipML5: Failed to resume AudioContext:', error);
      });
    }
  }
  ...
  enableAudio() {
    ...
    if (this.audioContext && this.audioContext.state === 'suspended') {
      this.audioContext.resume().catch(error => {
        console.warn('SipML5: Failed to enable audio:', error);
      });
    }
  }
  ...
  private setupConferenceMixer(): void {
    try {
      ...
      console.log('SipML5: Conference mixer setup complete');
    } catch (error) {
      console.error('SipML5: Failed to setup conference mixer:', error);
    }
  }
  ...
  private cleanupConferenceMixer(): void {
    if (this.conferenceMixer) {
      try {
        ...
        console.log('SipML5: Conference mixer cleaned up');
      } catch (error) {
        console.error('SipML5: Error cleaning up conference mixer:', error);
      }
    }
  }
  ...
  private generateRingtone() {
    try {
      console.log('SipML5: Starting ringtone generation');
      this.setupAudioContext();
      if (!this.audioContext) {
        console.error('SipML5: AudioContext not available for ringtone');
        return;
      }
      
      console.log('SipML5: AudioContext state:', this.audioContext.state);
  ...
  disableConferenceMode(): void {
    ...
    if (incomingCalls.length > 0 && outgoingCalls.length > 0) {
      ...
      outgoingCalls.forEach(call => {
        console.log('SipML5: Disconnecting outgoing call on conference exit:', call.sessionId);
      ...
      if (primaryCall.isOnHold) {
        try {
          ...
          console.log('SipML5: Resumed primary incoming call on conference exit:', primaryCall.sessionId);
        } catch (error) {
          console.error('SipML5: Failed to resume primary call on conference exit:', primaryCall.sessionId, error);
        }
      }
      ...
      otherCalls.forEach(call => {
        console.log('SipML5: Disconnecting secondary call on conference exit:', call.sessionId);
        this.endCall(call.sessionId);
      });
      ...
      if (primaryCall.isOnHold) {
        try {
          this.unholdCallBySessionId(primaryCall.sessionId);
          console.log('SipML5: Resumed primary call on conference exit:', primaryCall.sessionId);
        } catch (error) {
          console.error('SipML5: Failed to resume primary call on conference exit:', primaryCall.sessionId, error);
        }
      }
  ...
  addToConference(sessionId: string): boolean {
    if (!this.sessions.has(sessionId)) {
      console.warn('SipML5: Cannot add non-existent session to conference:', sessionId);
      return false;
    }

    if (this.conferenceParticipants.has(sessionId)) {
      console.warn('SipML5: Session already in conference:', sessionId);
      return false;
    }
    ...
    // Resume call if it was on hold and unmute
    const callInfo = this.callInfos.get(sessionId);
    if (callInfo) {
      if (callInfo.isOnHold) {
        try {
          this.unholdCallBySessionId(sessionId);
          console.log('SipML5: Resumed held call for conference addition:', sessionId);
        } catch (error) {
          console.error('SipML5: Failed to resume held call for conference addition:', sessionId, error);
        }
      }
      callInfo.isOnHold = false;
    ...
    console.log('SipML5: Added to conference:', sessionId);
    this.updateCallState();
    return true;
  }
  ...
  removeFromConference(sessionId: string): boolean {
    if (!this.conferenceParticipants.has(sessionId)) {
      console.warn('SipML5: Session not in conference:', sessionId);
      return false;
    }
    ...
    console.log('SipML5: Removed from conference:', sessionId);
    this.updateCallState();
    return true;
  }
